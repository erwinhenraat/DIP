...
public class Platformer extends ApplicationAdapter {
	...
	private void jump(float delta){
		...
		if(jumping) {
			...
			currentFrame = animations[1].getKeyFrame(fTime); 	//get frame from animation
		}
		if(posy <= yFloor){											//touch the floor
			jumping = false;										//change state to not jumping
			t = 0;													//reset jumptime
			currentFrame = animations[0].getKeyFrame(fTime);	//get frame from animation
		}
	}
	private void shoot(float delta){
		...
		{
			cooldown-=delta;
			currentFrame = animations[3].getKeyFrame(fTime);
		}
